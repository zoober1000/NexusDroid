...
def get_cpu_temp():
    ...
    for zone in sorted(glob.glob('/sys/class/thermal/thermal_zone*/temp')):
        ...
        if raw:
            try:
                t = float(raw)
                # Android reports millidegrees; Linux reports degrees
                if t > 1000:
                    t /= 1000.0
                best = max(best, t)
            except ValueError:
...
def get_top_processes(n=6):
    ...
    try:
        ...
        for pid in os.listdir('/proc'):
            ...
            # RSS in pages → KB (page size typically 4096 on Android)
            statm = _read_file(f'/proc/{pid}/statm')
            rss_pages = int(statm.split()[1]) if statm else 0
            rss_kb = rss_pages * 4          # 4 KB pages
    ...
    # Compute delta CPU % using stored previous tick counts
    prev = getattr(get_top_processes, '_prev', {})
    clk = os.sysconf('SC_CLK_TCK') if hasattr(os, 'sysconf') else 100
    interval = 2.0   # matches Clock.schedule_interval period
    for p in procs:
...
class DashboardScreen(Screen):
    ...
    def _update_stats(self, dt):
        ...
        # Storage  (shutil.disk_usage — stdlib, no deps)
